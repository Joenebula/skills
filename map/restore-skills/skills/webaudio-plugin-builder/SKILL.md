...
description: >-
  Build browser-based audio instruments and plugins — synths, samplers, drum machines, slicers,
  effect racks, sequencers, loopers, modular patch bays — as single-file HTML apps with a Web
  Audio engine, a transport, and a control surface. Use this skill whenever the user wants to
  build, extend, restyle, debug or refactor any audio tool that runs in a browser: "build me a
  synth", "make a sampler", "add a delay", "the sliders don't do anything", "the effect keeps
  playing when it's off", "make the grid sequence the effects", "why is it out of time", "add
  presets", "the rack won't animate". Also use it for the UI surface of such tools — panels,
  dials, sliders, step grids, patch cables, preset banks, help sheets — and whenever a plugin's
  audio graph, scheduler, or parameter automation is involved. It carries a working UI kit, a
...
3b. **`references/product-layer.md`** — what turns a working instrument into a *product*:
   persistence that survives a double-click, session restore, versioned presets + migration,
   replace-without-losing-work, A/B, MIDI learn, master limiter/meters — plus the workflow
   (audit → evolve, exact-anchor patching, sibling port via subagent). Read it whenever a
   build works and the job is to finish or ship it, or when starting plugin #2.
4. **`references/ui-kit.md`** — the control surface: what a section looks like, what a
   control owes the user, the components. **Two visual languages are available**: the light
   hardware kit here, and the dark boutique kit in **`references/nebula-ui-kit.md`**
   (machined knobs, glassy cards, glowing accents — with a living spec in
   `assets/nebula-preview.html`). Pick one per project; the control contract and the
   colour-means-block law are identical in both.
...
**A working build gets evolved, not rewritten.** When handed an existing plugin and a brief
to "make it real", audit first (measure: setters, persistence, `pushHistory` coverage,
duplicated paths — a holds/broken/missing table), then add what's missing through the
existing laws. A rewrite of a law-abiding build loses polish and re-earns old bugs. The
audit-then-evolve path shipped two plugins in one day; see `references/product-layer.md`.
