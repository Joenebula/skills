...
const BASIS={
 /* A demo, so the open state can be seen while everything real is settled. Delete these
    four lines — the BASIS, NEEDS, WHERE and FIXES entries keyed 'impeccable' — and the map
    goes back to 15 findings, all closed. Nothing else refers to it. */
 'impeccable':['confirmed',0],
...
const NEEDS={
 'impeccable':'decide','CLAUDE.md':'push','security':'command','security-route-auditor':'command',
...
const WHERE={
 'impeccable':'repo','CLAUDE.md':'workspace + repo','security':'workspace','security-route-auditor':'workspace + repo',
...
const FIXES={
 'impeccable':"DEMO — this one is not real. It exists so the open state can be seen while every genuine finding is settled. Delete the four 'impeccable' entries in skills-data.js (BASIS, NEEDS, WHERE, FIXES) and it goes.\n\nWhat a real finding looks like, using a real question as the example:\n\nBASIS: confirmed. impeccable arrived on 9 Aug and nothing in BUILD-PLAN.md section 10 places it, so the map guessed its column from its description — nearest match mydesignskill, which put it in Front-end design. The guess is on the node itself and says so.\n\nThat is a real open question rather than an error: the map cannot know which build step a skill belongs to, only which skills it reads like. Naming it in section 10 replaces the guess with your answer.\n\nSo the shape of every finding is the same three parts. What was observed, how good the evidence is, and what would close it — with the last one being something you can act on or argue with, not a task list.",
...
const GROUP_ABOUT={
  ...
  always:'Loaded on every request, before any task is chosen, because <code>CLAUDE.md</code> names them by hand. They cost you context in every single session, so the bar for adding one is high — and <code>refactoring</code> is here on the build plan\u2019s say-so while <code>CLAUDE.md</code> lists only three, which is one of the open items.',
