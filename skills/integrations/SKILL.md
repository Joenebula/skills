...
Sync jobs run via [[background-jobs]]; secrets and signatures are [[security]]; field ownership is a [[data-modelling]] decision; erasure must propagate ([[privacy-and-compliance]]).
...
- **Map fields deliberately** (their schema ↔ yours); don't assume names or shapes match — verify by reading their API ([[ask-dont-guess]]).
...
| Rule | Why |
|---|---|
| **Verify the signature** | an unsigned webhook is forgeable — reject it ([[security]]). |
...
- **Key-aware fallback**: a missing/invalid API key disables *that* feature cleanly with an honest message — the rest of the app keeps working ([[observability]], [[ask-dont-guess]]).
...
- Integration secrets are **environment-scoped**, never committed ([[security]], [[project-setup]]).
- **Propagate compliance**: erasure/rectification must reach synced third parties ([[privacy-and-compliance]]).
- An integration that's **not wired** is disabled/labelled, never a faked "connected" badge ([[ask-dont-guess]]).
...
- [[security]] — signed webhooks, environment-scoped secrets, never trust inbound data.
