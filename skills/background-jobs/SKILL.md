...
- **Scheduled**: nightly digests, retention purges ([[privacy-and-compliance]]), points/credit expiry ([[loyalty]]), sale activation, subscription renewals.
...
- Use an **idempotency key / processed-marker** so "send the email", "charge the card" ([[payments]]), "award the points" happen **once** even if delivered twice.
...
- [[payments]] — charge/refund jobs must be exactly-once via idempotency keys.
- [[email-and-notifications]] — queued sends, digests, and fan-out run here, exactly once.
- [[privacy-and-compliance]] / [[loyalty]] — retention purges, points expiry, lifecycle automation.
